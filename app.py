...
themes_css = {
    ...
    "Classic Studio 🎙️": """
        ...
        background-size: 40px 40px; color: #d1d8e0;
    """
}
...
if "Wooden Side-Panels 🪵" in purchases: rack_border = "8px solid #5d4037"
...
<style>
    ...
    section[data-testid="stSidebar"] {{ background: {rack_bg} !important; border-right: {rack_border} !important; {rack_glow} }}
...
with st.sidebar:
    ...
    if "Analog VU Meters 📈" in purchases:
        st.write("Input Level"); st.markdown('<div class="vu-meter"></div>', unsafe_allow_html=True)
    ...
    unlocked = ["Default Dark"] + (["Classic Studio 🎙️"] if "day1" in claimed else [])
    sel_theme = st.selectbox("Theme", unlocked, index=unlocked.index(active_theme) if active_theme in unlocked else 0)
    if sel_theme != active_theme: save_all(theme_to_save=sel_theme); st.rerun()
    st.write("**Enabled Gear**")
    for g in gear_items.keys():
        if g in purchases: st.caption(f"✔️ {g} Active")
...
with t_vau:
    # --- VAULT EDITING LOGIC ---
    # 1. Parse all valid dates from data
    data_dates = [datetime.strptime(d, '%d/%m/%Y').date() for d in entry_map.keys()]
    # 2. Add Today to ensure range covers up to now
    if today_date not in data_dates: data_dates.append(today_date)
    # 3. Determine Range (Min -> Max)
    min_d = min(data_dates)
    max_d = max(data_dates) # This captures future dates if they exist in file
    
    # 4. Iterate Backwards (Latest -> Earliest)
    curr_d = max_d
    while curr_d >= min_d:
        ...
        with st.expander(f"📅 {d_s} {'(Today)' if d_s == today_str else ''}", expanded=(d_s == today_str)):
            new_txt = st.text_area(f"Edit {d_s}", value=content, height=150, key=f"v_{d_s}")
            if st.button(f"Update {d_s}", key=f"btn_{d_s}"):
