...
shop_items = {"Coffee Machine ☕": 150, "Studio Cat 🐈": 400, "Neon 'VIBE' Sign 🏮": 800, "Bass Subwoofer 🔊": 1500, "Smoke Machine 💨": 2500, "Golden Mic 🎤": 5000, "Diamond Discs 💎": 10000}
...
quest_pool = [
    ...
    {"id": "q_streak", "desc": "3+ Day Streak", "req": current_streak >= 3, "rc": 75},
...
# --- 5. STYLING ---
themes = {"Default Dark": "background: #0f0f0f;", "Classic Studio 🎙️": "background: #1e272e;"}
gear_unlocks = ["Acoustic Foam 🎚️", "LED Strips 🌈", "Gold XLR Cable 🔌", "Pop Filter 🎙️"]
...
with st.sidebar:
    ...
    selected_theme = st.selectbox("Theme", unlocked_themes, index=0)
...
with t_car:
    ...
    sq_percent = len(enabled_gear) * 25
    st.write(f"**Sound Quality: {sq_percent}%**")
    st.progress(sq_percent / 100)
    
    st.divider()
    st.header("Achievements")
    achs = [{"id": "day1", "name": "Intern", "target": 1, "curr": active_sessions},
            {"id": "words_500", "name": "Wordsmith", "target": 500, "curr": total_words}]
    for a in achs:
        prog = min(a['curr'] / a['target'], 1.0)
        st.subheader(f"{a['name']}")
        st.progress(prog)
        if a['id'] in claimed: st.success("Claimed")
        elif prog >= 1.0 and st.button(f"Claim Reward", key=f"c_{a['id']}"):
